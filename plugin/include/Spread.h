 ...
 * doubler-spec.md for the full design rationale).
 *
 ...
 * through identity instead of leaving decaying lag on the old side (spec
 * routing note), and knob moves read as a tape-style varispeed glide.
 *
 ...
 * the spec pseudocode's fixed placeholder was orders of magnitude off.
 *
 * Spec clarification — center identity: the spec's pseudocode leaves the
 * allpass cascade + lag gain engaged at T = 0, but its testing checklist
 * requires L/R identical at the center detent. The lag path therefore blends
 * to the dry high band as |t| falls below kCenterBlendMs, making center
 * exactly dual-mono while sweeps stay continuous (the tape-flange zone
 * effectively starts around 1 ms).
 *
...
private:
  ...
  // ── Fixed design values (see doubler-spec.md for rationale) ──
